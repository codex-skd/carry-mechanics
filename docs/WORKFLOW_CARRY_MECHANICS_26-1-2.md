...
|---|---|---|
| **snake_case** | `mod_id` en gradle.properties, assets/, packages Java | `carry_mechanics` |
| **PascalCase** | Clases Java principales | `CarryMechanics` |
| **camelCase** | Variables, métodos, config keys | `carryMechanics` |
| **Title Case** | Display name en README, CHANGELOG, docs, CurseForge | `Carry Mechanics` |

### Ficheros de documentación

| Fichero | Formato | Ejemplo |
|---|---|---|
| WORKFLOW | `WORKFLOW_<MOD_ID>_<MC-VERSION>.md` | `WORKFLOW_CARRY_MECHANICS_26-1-2.md` |
...
- Las config keys en camelCase: `carryMechanics.enableFeature`
...
```
carry_mechanics/             # Carpeta organizativa, sin .git
...
minecraft/26.1.2/neoforge-26.1.2.78/production
  (código + docs/ + lib_ext/             minecraft/26.1.2/neoforge-26.1.2.78/main
   + graphify-out/ + tokens)              (solo código + libs/
        │                                   + README + placeholders)
        │  CI/CD: filtra, sanitiza,
        │  commitea con force push
        ▼  a la rama */main hermana
  minecraft/26.1.2/neoforge-26.1.2.78/main ──→ GitHub (mirror)
```
...
| Ejemplo | Significado |
|---|---|
| `carry_mechanics-26.1.2-neoforge-0.0.0-beta.27.jar` | NeoForge 26.1.2, beta 27 |

El framework se configura en `build.gradle`:
...
|---|---|---|
| Beta | `<mc-version>-neoforge-beta.X` | `26.1.2-neoforge-beta.27` |
...
```bash
# Beta
git tag -a 26.1.2-neoforge-beta.27 -m "v0.0.0-beta.27: Code cleanup and mixin fixes"
git push origin 26.1.2-neoforge-beta.27
...
2. Deriva la rama `main` hermana: `minecraft/26.1.2/neoforge-26.1.2.78/production` → `minecraft/26.1.2/neoforge-26.1.2.78/main`
...
#    cp build/libs/carry_mechanics-26.1.2-neoforge-0.0.0-beta.28.jar /ruta/a/la/instancia/mods/
#    rm /ruta/a/la/instancia/mods/carry_mechanics-26.1.2-neoforge-0.0.0-beta.27.jar
```
...
#    mod_version=0.0.0-beta.28

# 3. Compilar con clean
./gradlew.bat clean build

# 4. Crear release notes
#    docs/curseforge/versions/0.0.0-beta.28.md

# 5. Actualizar CHANGELOG.md

# 6. Commit del bump de versión
git add -A
git commit -m "chore: bump version to 0.0.0-beta.28"

# 7. Tag para CurseForge
git tag -a 26.1.2-neoforge-beta.28 -m "v0.0.0-beta.28: Bugfix release"
git push origin 26.1.2-neoforge-beta.28
...
- **Sin residuos de mod original**: si el mod está basado en otro mod existente (fork/referencia), no debe quedar ningún rastro accidental del mod original. Revisar:
  - Nombres de paquetes (`com/oldauthor/oldmod/` → `com/skd/carrymechanics/`)
