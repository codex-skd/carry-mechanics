...
| `display_name` | `Carry Mechanics` (separado, no junto) |
...
| Campo | Valor | Notas |
|-------|-------|-------|
| `displayName` | `Carry Mechanics (0.0.0-beta.16)` | Nombre visible: `display_name (version)` |
...
```html
<h2>v0.0.0-beta.16 - Titulo descriptivo</h2>
...
<p><strong>JAR</strong>: <code>carry_mechanics-26.1.2-neoforge-0.0.0-beta.16.jar</code></p>
```
...
metadata = {
    "displayName": f"Carry Mechanics ({version})",
    "changelog": "<h2>v0.0.0-beta.16 - Titulo</h2>",
    "changelogType": "html",
    "gameVersionNames": ["Client", "Server", "26.1.2", "NeoForge"],
    "releaseType": "release"
}

with open(f"build/libs/carry_mechanics-26.1.2-neoforge-{version}.jar", "rb") as f:
    jar_data = f.read()
...
body += meta_bytes + b"\r\n"
body += f"--{boundary}\r\n".encode()
body += b'Content-Disposition: form-data; name="file"; filename="carry_mechanics-26.1.2-neoforge-{version}.jar"\r\n'
body += b"Content-Type: application/java-archive\r\n\r\n"
body += jar_data + b"\r\n"
body += f"--{boundary}--\r\n".encode()

req = urllib.request.Request(
    f"https://minecraft.curseforge.com/api/projects/1608286/upload-file",
    data=body,
    headers={
...
```bash
curl -s "https://api.curseforge.com/v1/mods/1608286/files/<FILE_ID>"   -H "x-api-key: $2a$10$yGwryAfmRkS9ZJsJUDf5YOKZpOIsmHB8Fji2D8JVCKBSZEKYlwmaO"
```

## Changelog

```bash
curl -s "https://api.curseforge.com/v1/mods/1608286/files/<FILE_ID>/changelog"   -H "x-api-key: $2a$10$yGwryAfmRkS9ZJsJUDf5YOKZpOIsmHB8Fji2D8JVCKBSZEKYlwmaO"
```
