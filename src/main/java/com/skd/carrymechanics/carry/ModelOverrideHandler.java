...
public class ModelOverrideHandler {
    ...
    public static void parseAndAdd(String input) {
        try {
            ...
            Block block = null;
            var blockOpt = BuiltInRegistries.BLOCK.getOptional(new ResourceLocation(blockPart));
            ...
            if (renderSplit.length >= 2) {
                var itemOpt = BuiltInRegistries.ITEM.getOptional(
                        new ResourceLocation(renderSplit[0] + ":" + renderSplit[1]));
                if (itemOpt.isEmpty()) return;
                item = itemOpt.get();
                if (renderSplit.length > 2) type = renderSplit[2];
            } else {
                var itemOpt = BuiltInRegistries.ITEM.getOptional(new ResourceLocation(renderPart));
                if (itemOpt.isEmpty()) return;
                item = itemOpt.get();
                type = "item";
            }
