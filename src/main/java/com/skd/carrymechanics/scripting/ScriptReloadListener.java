...
public class ScriptReloadListener {
    ...
    public static void onResourceManagerReload(ResourceManager manager) {
        ...
        converter.listMatchingResources(manager).forEach((id, resource) -> {
            try (var reader = resource.openAsReader()) {
                ...
                ).getOrThrow(false, msg -> { throw new RuntimeException("Script parse: " + msg); });
                scripts.add(script);
            } catch (Exception e) {
    ...
    private static void syncToClient(ServerPlayer player) {
        Tag serialized = CarryScript.CODEC.listOf()
                ...
                .getOrThrow(false, msg -> { throw new RuntimeException("Failed to sync scripts: " + msg); });
        NetworkHelper.sendToPlayer(player, new ClientboundSyncScriptsPacket(serialized));
    }
}